...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    # parse ground truth
    if data_name in ["math","math500", "math_train", "minerva_math", "aops_forum", "aops_forum_15k", "orca_math_7.5k","cn_k12_7.5k","synthetic_math_7.5k"]:
        gt_cot = example["solution"]
        gt_ans = extract_answer(gt_cot, data_name)
    elif data_name in ["gsm8k", "gsm8k_train"]:
        ...
        gt_cot, gt_ans = None, example["final_answer"][0].replace("$", "").strip()
    elif data_name in [
        "aime24",
        "amc23",
        "cmath",
        ...
        "imo2024",
        "step_dpo10k",
        "relevant_benchmarkv1",
        "relevant_benchmarkv2",
        "relevant_benchmarkv3",
        "relevant_benchmark_mathstral"
    ]:
        gt_cot, gt_ans = None, example["answer"]
    else:
...
def parse_question(example, data_name):
    ...
    if data_name == "asdiv":
        ...
        options = " ".join(options)
        question = f"{example['question'].strip()}\n选项: {options}"
    elif data_name == "step_dpo10k":
        question = example["prompt"]
    else:
        for key in ["question", "problem", "Question", "input","instruction"]:
            if key in example:
